...
VoIPGeneratorOld *VoIPGeneratorOld::thisptr = (VoIPGeneratorOld *)NULL;

Define_Module(VoIPGeneratorOld);
...
void VoIPGeneratorOld::initialize(int stage)
{
    DIR *sound;
    char *name;
    VoIP_fileEntry *pkt;
    ...
    ev << "VoIPGeneratorOld -> initialize(" << stage << ")" << endl;
    VoIPGeneratorOld::thisptr = this;
    ...
    list = VoIPGeneratorOld::getWavInfo(namelist, noWavFiles);
...
void VoIPGeneratorOld::handleMessage(cMessage *msg)
{
...
char* VoIPGeneratorOld::itoa(int intVal)
{
...
VoIP_fileList *VoIPGeneratorOld::getList()
{
	return thisptr->traceList;
}


void VoIPGeneratorOld::finish()
{
    // say GOOD BYE...
    char name[255];
    ...
    {
	    if(traceList != NULL)
	        VoIPGeneratorOld::writeToDisk(traceList, name);
	    else 
	    {
...
VoIP_fileList *VoIPGeneratorOld::generateTrace(simtime_t sec_)
{
...
struct wavinfo **VoIPGeneratorOld::getWavInfo(struct dirent **namelist, int n)
{
...
void VoIPGeneratorOld::writeToDisk(VoIP_fileList *traceList, char *filename)
{
	int no,i;
	int current;
	VoIP_fileEntry *pkt;
